...
public class PublicVehiclesController {
    ...
    public void initialize() {
        ...
        isUserLoggedIn = sessionManager.estConnecte();

        if (isUserLoggedIn) {
            currentUser = sessionManager.getUtilisateurConnecte();
            currentClientId = currentUser.getIdUtilisateur();
            System.out.println("✅ Utilisateur connecté - ID: " + currentClientId);
        } else {
            System.out.println("👤 Utilisateur non connecté - Mode invité");
        }
        ...
        System.out.println("🚗 Initialisation du contrôleur véhicules");
        if (isUserLoggedIn) {
    ...
    private boolean checkLoginRequired(String featureName) {
        if (!isUserLoggedIn) {
            showLoginRequiredAlert(featureName);
            return false;
        }
        return true;
    }
    ...
    /**
     * ✅ Affiche une alerte demandant la connexion
     */
    private void showLoginRequiredAlert(String featureName) {
        Platform.runLater(() -> {
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setTitle("Connexion requise");
            alert.setHeaderText("Cette fonctionnalité nécessite une connexion");
            alert.setContentText("Vous devez vous connecter pour " + featureName + ".\n\nSouhaitez-vous vous connecter maintenant ?");
            ...
            ButtonType loginButton = new ButtonType("Se connecter", ButtonBar.ButtonData.OK_DONE);
    ...
    private void redirectToLogin() {
        try {
            Stage currentStage = (Stage) vehiclesGrid.getScene().getWindow();
    ...
    private VBox createModernVehicleCard(Vehicle vehicle) {
        ...
        // ✅ BOUTON FAVORI AVEC VÉRIFICATION DE CONNEXION
    ...
    private Button createAvailabilityButton(Vehicle vehicle) {
        ...
        boolean estReserveParMoi = false;

        if (isUserLoggedIn) {
            estReserve = reservationDAO.isVehiculeReserved(vehicle.getId());
            estReserveParMoi = reservationDAO.hasClientReservedVehicule(currentClientId, vehicle.getId());
        }
    ...
    private Button createFavoriteButton(Vehicle vehicle) {
        ...
        if (isUserLoggedIn) {
            isFav = favoriteDAO.isFavorite(currentClientId, vehicle.getId());
        }
        ...
        favoriteBtn.setOnAction(e -> {
            if (!checkLoginRequired("ajouter aux favoris")) {
                return;
            }
            toggleFavorite(vehicle, favoriteBtn);
        });
    ...
    /**
     * ✅ NOUVELLE MÉTHODE : Retour à tous les véhicules
     */
